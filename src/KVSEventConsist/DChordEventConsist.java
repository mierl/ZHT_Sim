...
public class DChordEventConsist
{
	public Node node;
	public PeerProtocol pp;
	
	public DChordEventConsist(Node node, PeerProtocol pp)
	{
		this.node = node;
		this.pp = pp;
	}
	...
	public void operaNodeFail(Object failEvent)
	{
		...
		if (failEvent.getClass() == OperaMessage.class)
		{
			...
			rcmc = new ResClientMessageChurn(om.qk.taskId, om.type, 
			...
			TaskDetail td = Library.taskHM.get(om.qk.taskId);
		...
		else if (failEvent.getClass() == DChordForwardMessage.class)
		{
			...
			rfm = new DChordResForwardMessage(fm.qk.taskId, fm.messageId, fm.type, 
	...
	{
		DChordOperaMessage com = new DChordOperaMessage(fm.qk, fm.type, fm.sender, fm.key, 
	...
	{
		pp.updateMaxFwdTime(Library.sendOverhead);
		DChordResForwardMessage orm = new DChordResForwardMessage(fm.qk.taskId, fm.messageId, 
	...
	public void rangeJudFailProc(DChordForwardMessage fm, BigInteger idKey, long startTime)
	{
		...
		if (nextNode == null)
		{
			...
			else
			{
				...
				rcmc = new ResClientMessageChurn(fm.qk.taskId, fm.type, fm.key, fm.value, false,
						true, 0, node.getIndex(), replicaId);
			}
		}
		else
		{ 
			fmNew = new DChordForwardMessage(fm.qk, fm.messageId, fm.type, node,
	...
	public void operaMsgProcess(OperaMessage om)
	{
		...
		else
		{
			...
			DChordForwardMessage fm = new DChordForwardMessage(om.qk, messageId, 
	...
	public void resFwdMsgNodeOn(DChordResForwardMessage rfm, int upStreamId)
	{
		...
		if (upStreamId >= pp.numServer)
		{
			if (rfm.success)
			{
				com = new DChordOperaMessage(new QuorumKey(rfm.taskId,1), rfm.type, Network.get(upStreamId), 
	...
	public void chordOperaMsgProcess(DChordOperaMessage com)
	{
		...
		if (!pp.isUp())
		{
			...
			rm = new DChordResMessage(com.qk.taskId, com.type, com.sender, com.key, value, false, 
						true, com.numHops, node.getIndex(), pp.replicaIds[0]);
			EDSimulator.add(Library.sendOverhead + Library.commOverhead, 
										rm, Network.get(com.servId), pp.par.pid);
			TaskDetail td = Library.taskHM.get(com.qk.taskId);
			td.taskQueuedTime = CommonState.getTime();
			td.taskEndTime = CommonState.getTime();
			return;
		}
		...
		pp.actOperaMsgProcessEvenConsit(com, node);
	}
	...
	public void chordResMsgProcess(DChordResMessage rm)
	{
		...
		if (!rm.success)
		{
			DChordOperaMessage dom = new DChordOperaMessage(new QuorumKey(rm.taskId, 1), rm.type, rm.sender, 
					rm.key, rm.value, node.getIndex(), rm.numHops, false);
			int target = rm.curId, time = 0;
		...
		{
			ResClientMessageChurn rcmc = new ResClientMessageChurn(rm.taskId, rm.type, rm.key, 
				rm.value, rm.success, false, rm.numHops, node.getIndex(), pp.replicaIds[0]);
			if (pp.updateGlobalQueue(node, startTime, endTime, rcmc, rm.sender))
			{
				pp.numFwdMsg++;
			}
		}
	...
	public void joinNotMsgProcess(JoinNotifyMessage jnm)
	{
		...
		if (pp.versionedData.containsKey(jnm.joinId))
		{
	...
	public void processEvent(Object event)
	{
		...
		else if (event.getClass() == ResClientMessageChurn.class)
		{
			...
			pp.resClientMsgProcessRepChurn(node, Library.file + "/summary_client_0_" + 
					...
					pp.numReplica + "_" + Library.churnInterval + ".txt", rcmc);
		}
