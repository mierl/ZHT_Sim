...
public class Library 
{
	...
	public static void initLib(int numServer, int numOpera, int numReplica, int numClientPerServ)
	{
		...
		{
			case 10:
				Library.file = "./dfcevenconsist";
				Library.numClientPerServ = numClient / numServer;
				break;
			case 11:
				Library.file = "./dchordevenconsist";
				Library.numClientPerServ = numClient / numServer;
				break;
		}
		...
		try
		{
			...
			Library.bwTask = new BufferedWriter(new FileWriter(file + "/task_" + Integer.toString(
										numClient) + "_" + numReplica + "_" + 
										Library.churnInterval + ".txt"));
		...
		Library.waitQueue = new HashMap<Integer, 
							LinkedList<WaitingEvent>>(offset);
		Library.waitQueueConsist = new HashMap<Integer, 
				LinkedList<WaitingEvent>>(offset);
	...
	public static void logTask(TaskDetail td)
	{
		try
		{
			Library.bwTask.write(td.taskId + "\t" + (td.clientId - Library.offset) + "\t" + td.taskSuccess + "\t" 
								+ td.taskType + "\t" + td.taskSubmitTime + "\t"
								+ td.taskQueuedTime + "\t" + td.taskEndTime + "\t" 
								+ td.taskBackClientTime + "\t" + td.numHops + "\r\n");
	...
	public static void calServThroughput(int pid)
	{
		for (int i = 0;i < Library.offset;i++)
		{
			...
			pp.numAllReqFailed = pp.numLocalReqFailed + pp.numOtherReqFailed;
	...
	public static void printServStatistics(int pid)
	{
		...
		try
		{
			Library.bwSumStat.write(Double.toString(minThr) + "\t" + 
				Double.toString(sumThr / Library.offset) + "\t" + Double.toString(maxThr) + "\t");
		}
		catch (IOException e)
		{
			e.printStackTrace();
	...
	public static void logClientStatistics(int numClient, int pid) 
																throws IOException
	{
		for (int i = 0;i < numClient;i++)
		{
	...
	public static void printLoad(int pid)
	{
		...
		try
		{
			Library.bwSumStat.append(Long.toString(minLoad) + "\t" + 
								Double.toString((double)sumLoad / (double)Library.offset) + "\t" + 
								Double.toString(maxLoad) + "\t");
		}
		catch (IOException e)
		{
			e.printStackTrace();
	...
	public static void processEvent(Node node, int pid, Object event)
	{
		...
		{
			case 10:
				new DFCEventConsist(node, pp).processEvent(event);
				break;
			case 11:
				new DChordEventConsist(node, pp).processEvent(event);
				break;
		}
	}
	...
	public static void dchordChurn(boolean type, Node newNode, int idLength, int pid)
	{
		...
		if (Library.eager)
		{
			...
			for (int i = 1;i <= idLength;i++)
			{
				...
				{
					EDSimulator.add(pp.waitTimeCal(pp.maxFwdTime), new UFTMessage(
					type, i, newNode.getIndex(), pp.succList[0].getIndex()), pp.predecessor, pid);
				}
				else
				{
