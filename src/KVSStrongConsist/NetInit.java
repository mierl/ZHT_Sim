...
public class NetInit implements Control 
{
	...
	public void cTreeInit()
	{
		int numServerExtend = numServer + numReplica;
		...
		for (int i = 0;i < Network.size();i++)
		{
			...
			PeerProtocol pp = (PeerProtocol)node.getProtocol(pid);
			pp.id = new BigInteger(Integer.toString(i));
			if (i >= numServerExtend)
			{
				pp.servId = (i - numServerExtend)  / numClientPerServ + offset;
			}
			pp.al = new ArrayList<OperaMessage>();
			pp.alBackUp = new ArrayList<OperaMessage>();
			pp.count = 0;
			pp.memList = new HashMap<Integer, Boolean>();
			pp.setUp();
		}
	...
	public void overallInit()
	{
		for (int i = 0;i < Network.size();i++)
		{
			...
			if (i < Library.offset)
			{
				...
				pp.hmAllData = new HashMap<Integer, HashMap<String, String>>();
				pp.hmAllData.put(i, new HashMap<String, String>());
				...
				for (int j = 0;j < numReplica;j++)
				{
					...
					else if (Library.type == 9)
					{
						pp.replicaIds[j] = pp.succList[j].getIndex();
					}
					pp.hmAllData.put(pp.replicaIds[j], new HashMap<String, String>());
				}
				pp.numFwdMsg = 0;
			...
			pp.hmRepTry = new HashMap<Long, Integer>();
	...
	public boolean execute()
	{
		...
		if (Library.type >= 4)
		{
			...
			EDSimulator.add(Library.currChurnTime, new ChurnNoticeMessage(), 
													node, pid);
		}
		return false;
	}
